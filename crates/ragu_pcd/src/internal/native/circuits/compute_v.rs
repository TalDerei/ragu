...
use ragu_primitives::{Element, Endoscalar, GadgetExt, allocator::StubAllocator};
    ...
    for Circuit<C, R, HEADER_SIZE>
{
        ...
        Self: 'dr,
    {
        ...
        let w = unified_output.w.read(dr, &mut allocator)?;
        let y = unified_output.y.read(dr, &mut allocator)?;
        let z = unified_output.z.read(dr, &mut allocator)?;
        let x = unified_output.x.read(dr, &mut allocator)?;
        ...
        // the claimed evaluation.
        {
            ...
            let fu = {
                let alpha = unified_output.alpha.read(dr, &mut allocator)?;
                let u = unified_output.u.read(dr, &mut allocator)?;
            ...
            let computed_v = {
                let pre_beta = unified_output.pre_beta.read(dr, &mut allocator)?;
