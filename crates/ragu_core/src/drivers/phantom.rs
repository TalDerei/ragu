...
/// Driver types for the zero-cost dummy [`Driver`] implementation on
/// `PhantomData<F>`: `Wire = ()`, `MaybeKind = Empty`, and unit linear
/// combinations. See the [`Driver`] implementation for the rationale.
...
/// Zero-cost dummy driver with `Wire = ()` and `MaybeKind = Empty`. All
/// constraint methods are no-ops, and witness closures are never called — the
/// compiler dead-code eliminates them entirely.
///
/// This implementation exists for three reasons:
///
