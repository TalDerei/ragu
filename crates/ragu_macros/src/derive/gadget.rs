...
fn test_gadget_derive_default_gadget() {
    ...
    // Conservative equality is no longer generated; it is derived once from
    // the canonical map_gadget traversal (see
    // `Gadget::enforce_conservative_equal`).
    assert!(
        !result_str.contains("enforce_conservative_equal"),
        "enforce_conservative_equal_gadget should no longer be generated by the derive"
    );
