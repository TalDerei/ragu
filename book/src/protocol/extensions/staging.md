...
In order to enforce this, we use a special "bonding polynomial" (a stage
mask) that performs a well-formed check on each of the stage polynomials.
The bonding polynomial is defined by the start and size of the portion of
the partial trace that is reserved for that polynomial; in order to be
safe, all wires in $a(X)$ should be set to zero if they are not within
this range. The bonding polynomial simply enforces that everything must be
nonzero in this range via simple linear constraints.
...
where $\v{s}$ is the bonding polynomial. Notice, this is expressly different from the traditional revdot claim that would be seen in a full circuit evaluation, which takes the form
...
because the linear constraints encoded in $\v{s}$ via the bonding polynomial
are only `enforce_zero` and **there are no public inputs for well-formedness
checks**.
