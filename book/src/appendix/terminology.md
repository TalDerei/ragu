...
| Witnessless wiring polynomial encoding only linear constraints, with zero constant term; used for stage well-formedness checks and cross-stage routing (batched in the revdot check) | **Bonding polynomial** | `Stage::mask()` or `Stage::final_mask()` |
