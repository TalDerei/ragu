...
| `clippy.toml` | production `ragu_pcd` computation cannot bypass `Backend` by calling canonical methods directly; CI selects this config with `CLIPPY_CONF_DIR` and scopes it to that crate with `--no-deps` | edit the method list deliberately |
