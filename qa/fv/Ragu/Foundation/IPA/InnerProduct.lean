...
import Ragu.Foundation.IPA.Basic

/-!
Ported from `Zcash/Snark/Soundness/Ipa/InnerProduct.lean` at `zcash/ironwood` commit
`3c056cbebf2880b54f801c348cb67ce7dc9f2a05`. Ragu adapts the namespace and replaces the broad
arithmetic import with the local generic URS plus direct generic algebra imports.
-/
...
theorem ipaRelation_unblind_value (urs : URS G) (P : G) (b : Fin (2 ^ urs.k) → F) (v ξ : F)
  ...
  obtain ⟨hc, hi⟩ := h
  have hsub : a - ξ • s = a + (-ξ) • s := by
    funext i
    simp [sub_eq_add_neg]
  refine ⟨?_, ?_⟩
  · rw [hsub, commit_add, commit_smul, hc, neg_smul]; abel
  · rw [hsub, innerProduct_add, innerProduct_smul, hi]; ring
