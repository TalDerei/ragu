...
/-- Evaluation commutes with the linear-combination fold. Stated over bare
indexed families so it applies to both the `linComb` and MDS-row cases. -/
...
/-- A symbolic linear combination evaluates to the value-level one. -/
...
/-- Value-level `add_round_constants`. -/
...
/-- The MDS layer commutes with evaluation. `normalizeLinear` is invisible
here precisely because it preserves values. -/
...
/-- The S-box wires this round allocates: three `Element.Mul` gates per state
word, whose result is the third wire of the third gate. Declared explicitly so
that the output is manifestly a function of the offset alone — that is what
makes the round `ConstantOutput`, and so usable as a `Circuit.foldl` body. -/
...
instance elaborated (P : Params (F p) t) : ElaboratedCircuit (F p) (fields t) (fields t) (main P) where
  ...
  output_eq state offset := by
    simp only [main, circuit_norm, Sbox.circuit, sboxWires]
    congr 1
    ext i hi
    simp [circuit_norm]
...
theorem soundness (P : Params (F p) t) :
  ...
  ext i hi
  simp only [Vector.getElem_map, sboxAllVal, sboxWires, Vector.getElem_ofFn, Expression.eval]
  exact h_holds ⟨i, hi⟩
