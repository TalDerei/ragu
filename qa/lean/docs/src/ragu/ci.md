...
Concretely, it first runs `cargo run --locked --manifest-path qa/crates/lean_extraction/Cargo.toml -- check`, which enforces that the checked-in generated Lean files — the `Ragu/Instances.lean` import root and the `Ragu/Fingerprint/Instances.lean` instance list — are up to date with respect to the exporter's target table.
...
Finally, CI runs the [fingerprint equivalence check](./fingerprint.md): it compares the canonical trace digests printed by `cargo run --locked --manifest-path qa/crates/lean_extraction/Cargo.toml -- fingerprint` against the ones computed in Lean from the `Clean` reimplementations (`lake env lean --run Ragu/Fingerprint/Main.lean`), and fails on any mismatch.
