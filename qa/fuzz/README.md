...
| `fuzz_internal_circuits` | The patcher aimed at the **production internal recursion circuits**. Captures all five native circuits from one real fuse (paid once, in a `LazyLock`), then pins each circuit's *received* instance wires and stage wires, cheats any other free advice, repairs through the captured constraints, and demands that no *computed* instance wire moved. For the hash circuits that is Fiat–Shamir binding; elsewhere it is an output the constraints must pin. |
